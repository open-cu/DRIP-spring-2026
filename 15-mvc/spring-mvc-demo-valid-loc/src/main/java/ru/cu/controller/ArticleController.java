...
public class ArticleController {
    ...
    public String saveArticle(@Valid @ModelAttribute("updateDto") ArticleForUpdateDto updateDto,
                                BindingResult bindingResult, Model model) {
        if (bindingResult.hasErrors()) {
            model.addAttribute("updateDto", updateDto);
            if (isNull(updateDto.getId())) {
                return showNewArticleForm(model);
            }
            return showEditArticleForm(updateDto.getId(), model);
        }
        articleService.save(updateDto);
